...
Elasticsearch can be used as a business analytics and in-sighting tool. 
  ...
  ##### Maps
Elasticsearch can be used to explore location and geoposition data in real time. 
...
Elasticsearch is built on Apache Lucene and it is the heart of the Elastic Stack. It is optimized for speed and relevance on production-scale workloads. 
...
## Logstash
Logstash is a data processing pipeline that collects data from various sources in real time, transform, and send the data. Logstash is often used to prepare data for analysis in Elasticsearch. 
...
## Kibana
Kibana is primarily used for data visualization and exploration. It allows users to create dashboards and interactive charts based on data stored on Elasticsearch. Kibana is thus a valuable tool for analyzing large volumes of logs, for monitoring applications, and for gaining operational insights through visual representations like line graphs, bar charts, heat maps, etc. 
...
Add the example log below. You can also use other logs or dataset to populate the access_log file. The example log (obtained from: https://medium.com/@swinarah/run-logstash-in-ubuntu-1fc3a4638ff6 ) is also downloadable from the attached access_log file on this Github page:
